...
// --- CẤU HÌNH GOOGLE SHEETS THÔNG MINH ---
// Nếu chạy trên Render (có biến GOOGLE_CREDS), nó sẽ dùng biến đó.
// Nếu chạy trên máy local (không có biến đó), nó mới tìm file credentials.json
...
try {
    if (process.env.GOOGLE_CREDS) {
        ...
        // Chỉ chạy dòng này nếu đang ở máy tính cá nhân và có file JSON
...
const commands = [
    new SlashCommandBuilder()
        .setName('tangxu')
        .setDescription('Tặng Sea Coins cho người chơi khác')
        .addUserOption(o => o.setName('user').setDescription('Người nhận').setRequired(true))
        .addIntegerOption(o => o.setName('amount').setDescription('Số xu muốn tặng').setRequired(true)),
    new SlashCommandBuilder()
        .setName('bxh')
        .setDescription('Xem bảng xếp hạng Top 10 người giàu nhất server'),
    new SlashCommandBuilder()
        .setName('batdau')
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        if (commandName === 'tangxu') {
            const target = options.getUser('user');
            const amount = options.getInteger('amount');
            const senderId = user.id;

            if (target.id === senderId) return interaction.reply({ content: '❌ Bạn không thể tặng xu cho chính mình!', ephemeral: true });
            if (amount <= 0) return interaction.reply({ content: '❌ Số xu tặng phải lớn hơn 0!', ephemeral: true });
            if ((seaCoinsBalances[senderId] || 0) < amount) return interaction.reply({ content: '❌ Bạn không đủ số dư!', ephemeral: true });

            seaCoinsBalances[senderId] -= amount;
            seaCoinsBalances[target.id] = (seaCoinsBalances[target.id] || 0) + amount;

            // Đồng bộ lên Google Sheets
            await updateBalance(senderId, -amount);
            await updateBalance(target.id, amount);

            return interaction.reply(`✅ Đã tặng **${amount} Sea Coins** cho <@${target.id}> thành công!`);
        }

        if (commandName === 'bxh') {
            const sorted = Object.entries(seaCoinsBalances)
                .sort((a, b) => b[1] - a[1])
                .slice(0, 10);
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(desc || 'Chưa có dữ liệu người chơi.');
